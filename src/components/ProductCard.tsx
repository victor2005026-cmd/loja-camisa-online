...
export function ProductCard({ camisa }: { camisa: CamisaComDetalhes }) {
  ...
  return (
    <div className="group flex flex-col overflow-hidden rounded-xl border border-line bg-surface transition-all duration-200 hover:-translate-y-1 hover:border-ouro hover:shadow-[0_12px_32px_-12px_rgba(201,162,74,0.35)]">
      <Link href={`/produto/${camisa.id}`} className="relative block aspect-square w-full overflow-hidden">
        {camisa.foto_url ? (
          <>
      ...
      <div className="flex items-center gap-1.5 border-b border-line p-1.5">
        <Link
          ...
          className="flex-1 rounded-full bg-flare px-2 py-1.5 text-center text-[11px] font-semibold text-ink transition hover:brightness-110"
        >
          Comprar
        </Link>
        <Link
          href={`/produto/${camisa.id}`}
          className="flex-1 rounded-full border border-line px-2 py-1.5 text-center text-[11px] font-semibold text-paper transition hover:border-ouro"
        >
          Saiba mais
        </Link>
        <a
          ...
          className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-[#25D366] text-white transition hover:brightness-110"
        >
